<template>
    <section class="featured-categories-wrap">
        <div class="container">
            <div class="featured-categories-header">
                ...
                <ul class="tabs featured-categories-tabs">
                    <li v-for="(tab, index) in tabs" :key="index" :class="classes(tab)" @click="change(tab)">
        ...
        <dynamic-tab v-for="(category, index) in data.categories" :key="index" :label="category.name"
...
export default {
    ...
    methods: {
        ...
        openModal(productImage, product) {
            this.$emit('open-modal', productImage, product); // Emit the event with the product image URL
        },
